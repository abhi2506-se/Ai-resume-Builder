...
export async function dbGetContactMessages(limit = 50) {
  const db = await getDB()
  return db`SELECT * FROM contact_messages WHERE archived = FALSE ORDER BY created_at DESC LIMIT ${limit}`
}
...
export async function dbArchiveContactMessage(id: string) {
  const db = await getDB()
  // archived column may not exist – add it if missing
  try {
    await db`ALTER TABLE contact_messages ADD COLUMN IF NOT EXISTS archived BOOLEAN DEFAULT FALSE`
    await db`ALTER TABLE contact_messages ADD COLUMN IF NOT EXISTS read_at BIGINT DEFAULT NULL`
  } catch {}
  await db`UPDATE contact_messages SET archived = TRUE WHERE id = ${id}`
}
export async function dbMarkContactMessageRead(id: string) {
  const db = await getDB()
  try {
    await db`ALTER TABLE contact_messages ADD COLUMN IF NOT EXISTS read_at BIGINT DEFAULT NULL`
  } catch {}
  await db`UPDATE contact_messages SET read_at = ${Date.now()} WHERE id = ${id} AND read_at IS NULL`
}
export async function dbGetUnreadContactCount() {
  const db = await getDB()
  try {
    await db`ALTER TABLE contact_messages ADD COLUMN IF NOT EXISTS read_at BIGINT DEFAULT NULL`
    await db`ALTER TABLE contact_messages ADD COLUMN IF NOT EXISTS archived BOOLEAN DEFAULT FALSE`
  } catch {}
  const res = await db`SELECT COUNT(*) as count FROM contact_messages WHERE read_at IS NULL AND archived = FALSE`

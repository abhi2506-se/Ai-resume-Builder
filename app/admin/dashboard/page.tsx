...
import {
  ...
  UserCircle, Camera, BarChart2, MessageSquare, Phone, Bot, Download, Bell, Sparkles, Brain,
  Rocket, Globe, Zap, TrendingUp, Star, Target,
} from 'lucide-react'
...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    ...
    fetch('/api/contact?type=unread')
      .then(r => r.json())
      .then(d => setGlobalUnread(d.count || 0))
      .catch(() => {})
  }, [])
          ...
          >
              ...
              {navItems.map((item) => {
                ...
                return (
                  ...
                  >
                    ...
                    {item.label}
                    {item.id === 'messages' && globalUnread > 0 && (
                      <span className="ml-auto flex items-center justify-center w-5 h-5 rounded-full bg-blue-500 text-white text-[10px] font-bold animate-pulse">
            ...
            <div className="px-3 py-4 border-t border-slate-700/40 space-y-2">
              <a
                href="/resume"
                target="_blank"
                className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm bg-gradient-to-r from-purple-600/10 to-blue-600/10 border border-purple-500/20 text-purple-300 hover:text-purple-200 hover:from-purple-600/20 hover:to-blue-600/20 transition-all"
              >
    ...
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-br from-purple-500/10 to-blue-500/10 border border-purple-500/20 rounded-2xl p-6">
        <div className="flex items-start gap-4">
          <div className="p-3 rounded-2xl bg-gradient-to-br from-purple-600 to-blue-600 shadow-lg flex-shrink-0">
            <Brain className="w-6 h-6 text-white" />
          </div>
          <div className="flex-1">
            <div className="flex items-center gap-2 mb-1">
              <h3 className="font-bold text-white">AI Resume Studio</h3>
              <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-purple-500/20 text-purple-300 border border-purple-500/20">LIVE</span>
            </div>
            <p className="text-sm text-slate-400 mb-4">Your portfolio now includes a full AI-powered resume analyzer and builder. Visitors can analyze their resumes against 17 top MNCs and build ATS-optimized resumes for free.</p>
            <div className="grid grid-cols-3 gap-3 mb-4">
              {[
                { icon: Target, label: '17 Companies', sub: 'Google, Amazon, TCS & more' },
                { icon: Zap, label: '90+ ATS Score', sub: 'AI keyword optimization' },
                { icon: Download, label: 'Free Downloads', sub: 'PDF, HTML, TXT' },
              ].map(f => (
                <div key={f.label} className="bg-white/[0.04] border border-white/[0.06] rounded-xl p-3">
                  <f.icon className="w-4 h-4 text-purple-400 mb-1.5" />
            ...
            <a href="/resume" target="_blank"
              className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-gradient-to-r from-purple-600 to-blue-600 hover:from-purple-500 hover:to-blue-500 text-white text-sm font-semibold transition-all">
              <Rocket className="w-3.5 h-3.5" /> Open AI Resume Studio
...
function MessagesSection() {
  ...
  const [loading, setLoading] = useState(true)
  const [filter, setFilter] = useState<'all' | 'hiring' | 'general' | 'archived'>('all')
  const [archivedMsgs, setArchivedMsgs] = useState<any[]>([])
  ...
  const load = async () => {
    setLoading(true)
    try {
      const [listRes, sumRes, archRes, unreadRes] = await Promise.all([
        fetch('/api/contact').then(r => r.json()),
        fetch('/api/contact?type=summary').then(r => r.json()),
        fetch('/api/contact?type=archived').then(r => r.json()).catch(() => ({ data: [] })),
        fetch('/api/contact?type=unread').then(r => r.json()).catch(() => ({ count: 0 })),
      ])
      setMsgs(listRes.data || [])
      setSummary(sumRes.data)
      setArchivedMsgs(archRes.data || [])
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      setArchivedMsgs(m => m.filter(x => x.id !== id))
    } catch {}
    setDeletingId(null)
  }
  ...
  const handleArchive = async (id: string) => {
    setArchivingId(id)
    try {
      await fetch('/api/contact', { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id, action: 'archive' }) })
      const msg = msgs.find(m => m.id === id)
      setMsgs(m => m.filter(x => x.id !== id))
      if (msg) setArchivedMsgs(a => [{ ...msg, archived: true }, ...a])
    } catch {}
    setArchivingId(null)
  }

  const handleMarkRead = async (id: string) => {
    await fetch('/api/contact', { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id, action: 'read' }) }).catch(() => {})
    setMsgs(m => m.map(x => x.id === id ? { ...x, read_at: Date.now() } : x))
  ...
  const handleNotifClick = () => {
    ...
    setFilter('all')
    // Mark all as read
    msgs.filter(m => !m.read_at).forEach(m => handleMarkRead(m.id))
  }

  const activeList = filter === 'archived' ? archivedMsgs : (filter === 'all' ? msgs : msgs.filter((m: any) => m.intent === filter))
  ...
  return (
    <div className="space-y-6">
          ...
          <AnimatePresence>
            {unread > 0 && !notifSeen && (
              <motion.button
                ...
                className="relative flex items-center gap-2 px-4 py-2 bg-blue-600/20 hover:bg-blue-600/30 border border-blue-500/40 rounded-xl text-sm text-blue-300 transition-all"
              >
                <Bell className="w-4 h-4" />
                <span className="font-semibold">{unread} new</span>
                <span className="absolute -top-1.5 -right-1.5 w-4 h-4 rounded-full bg-blue-500 text-white text-[10px] flex items-center justify-center font-bold animate-pulse">
                  {unread}
                </span>
              </motion.button>
            )}
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: 'Total Messages', value: summary?.total ?? 0, icon: MessageSquare, color: 'from-blue-600 to-cyan-500' },
          { label: 'Today', value: summary?.today ?? 0, icon: Calendar, color: 'from-green-600 to-teal-500' },
          { label: 'Hiring Intent', value: summary?.hiring ?? 0, icon: Briefcase, color: 'from-orange-600 to-red-500' },
          { label: 'Unique Senders', value: summary?.unique ?? 0, icon: BarChart2, color: 'from-purple-600 to-pink-500' },
        ].map(stat => {
          const Icon = stat.icon
          return (
            <div key={stat.label} className="bg-slate-900/60 border border-slate-700/40 rounded-2xl p-5">
      ...
      {/* Filter tabs */}
      <div className="flex gap-2 flex-wrap">
        {(['all', 'hiring', 'general', 'archived'] as const).map(f => (
          <button key={f} onClick={() => setFilter(f)}
            className={`px-4 py-1.5 rounded-full text-sm font-medium transition-all ${
              filter === f
                ? f === 'archived' ? 'bg-amber-600 text-white' : 'bg-blue-600 text-white'
                : 'bg-slate-800 text-slate-400 hover:text-white border border-slate-700'
            }`}>
            {f === 'all' ? `All (${msgs.length})` : f === 'hiring' ? '💼 Hiring Intent' : f === 'general' ? '💬 General' : `🗂 Archived (${archivedMsgs.length})`}
          </button>
        ))}
      </div>
      ...
      <div className="bg-slate-900/60 border border-slate-700/40 rounded-2xl overflow-hidden">
        {activeList.length === 0 ? (
          <div className="py-16 text-center text-slate-500">
            ...
            <p>{filter === 'archived' ? 'No archived messages.' : 'No messages yet. Share your portfolio to start receiving messages!'}</p>
          </div>
        ) : (
          <div className="divide-y divide-slate-700/30">
            <AnimatePresence>
              {activeList.map((msg: any) => (
                <motion.div
                  ...
                  transition={{ duration: 0.25 }}
                  className={`px-6 py-4 hover:bg-slate-800/30 transition-colors relative ${!msg.read_at && filter !== 'archived' ? 'border-l-2 border-blue-500' : ''}`}
                  onClick={() => { if (!msg.read_at) handleMarkRead(msg.id) }}
                >
                  <div className="flex items-start justify-between gap-4 mb-1.5">
                    <div className="flex items-center gap-2 flex-wrap">
                      {!msg.read_at && filter !== 'archived' && (
                    ...
                    <div className="flex items-center gap-2 flex-shrink-0">
                      ...
                      {filter !== 'archived' && (
                        <button
                          ...
                          className="p-1.5 rounded-lg text-slate-500 hover:text-amber-400 hover:bg-amber-400/10 transition-all disabled:opacity-40"
                        >
                          {archivingId === msg.id ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Download className="w-3.5 h-3.5" />}
                        </button>
                      )}
                      {/* Delete button */}
                      <button
                      ...
                      >
                        {deletingId === msg.id ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Trash2 className="w-3.5 h-3.5" />}
                      </button>
                    </div>
                  </div>
                  <a href={`mailto:${msg.email}`} className="text-xs text-blue-400 hover:underline mb-2 block" onClick={e => e.stopPropagation()}>{msg.email}</a>
